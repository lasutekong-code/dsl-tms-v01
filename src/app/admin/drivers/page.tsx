...
export default async function AdminDriversListPage({ searchParams }: PageProps) {
  ...
  let query = supabase.from("drivers").select("*", { count: "exact" }).order("created_at", { ascending: false }).range(from, to);
  if (q) {
  ...
  return (
    <div className="space-y-6">
      <AdminPageHeader title="운전자 관리" description="운전자 기본정보를 관리합니다." />
      <AdminDataTableShell
        toolbar={
          <>
      ...
      >
        ...
        <div className="flex items-center justify-between text-sm text-slate-600">
          ...
          <div className="flex gap-2">
            {page > 1 ? (
              <Button asChild size="sm" variant="outline">
                <Link href={`/admin/drivers?page=${page - 1}${q ? `&q=${encodeURIComponent(q)}` : ""}`}>이전</Link>
              </Button>
            ) : null}
            {page < totalPages ? (
              <Button asChild size="sm" variant="outline">
                <Link href={`/admin/drivers?page=${page + 1}${q ? `&q=${encodeURIComponent(q)}` : ""}`}>다음</Link>
              </Button>
            ) : null}
          </div>
        </div>
