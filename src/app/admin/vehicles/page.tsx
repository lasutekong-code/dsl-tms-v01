...
export default async function AdminVehiclesListPage({ searchParams }: PageProps) {
  ...
  let query = supabase.from("vehicles").select("*", { count: "exact" }).order("created_at", { ascending: false }).range(from, to);
  if (q) {
    query = query.ilike("vehicle_no", `%${q}%`);
  }
  ...
  return (
    <div className="space-y-6">
      <AdminPageHeader title="차량 관리" description="차량 기본정보를 관리합니다." />
      <AdminDataTableShell
        toolbar={
          <>
      ...
      >
        ...
        <div className="flex items-center justify-between text-sm text-slate-600">
          ...
          <div className="flex gap-2">
            {page > 1 ? (
              <Button asChild size="sm" variant="outline">
                <Link href={`/admin/vehicles?page=${page - 1}${q ? `&q=${encodeURIComponent(q)}` : ""}`}>이전</Link>
              </Button>
            ) : null}
            {page < totalPages ? (
              <Button asChild size="sm" variant="outline">
                <Link href={`/admin/vehicles?page=${page + 1}${q ? `&q=${encodeURIComponent(q)}` : ""}`}>다음</Link>
              </Button>
            ) : null}
          </div>
        </div>
