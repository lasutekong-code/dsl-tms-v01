...
export default async function AdminClientContactsPage({ searchParams }: PageProps) {
  ...
  const [{ data: rows, count, error }, { data: clients }] = await Promise.all([
    (() => {
      let query = supabase
  ...
  return (
    <div className="space-y-6">
      <AdminPageHeader title="담당자 관리" description="거래처·센터 담당자를 관리합니다." />
      <AdminDataTableShell
        toolbar={
          <>
            <AdminSearchBar defaultValue={q} />
            <AdminRegisterButton href="/admin/client-contacts/new" />
          </>
        }
      >
        <Table>
          ...
          <div className="flex gap-2">
            {page > 1 ? (
              <Button asChild size="sm" variant="outline">
                <Link href={`/admin/client-contacts?page=${page - 1}${q ? `&q=${encodeURIComponent(q)}` : ""}`}>이전</Link>
              </Button>
            ) : null}
            {page < totalPages ? (
              <Button asChild size="sm" variant="outline">
                <Link href={`/admin/client-contacts?page=${page + 1}${q ? `&q=${encodeURIComponent(q)}` : ""}`}>다음</Link>
              </Button>
            ) : null}
          </div>
        </div>
