...
import { flattenZodErrors, requiredTrimmed } from "@/lib/admin/zod-util";
...
const updateSchema = z.object({
  ...
  content: requiredTrimmed("메모 내용").optional(),
...
export async function PATCH(request: NextRequest, context: RouteContext) {
  ...
  const { data, error } = await supabase.from("memos").update(patch as Partial<MemoRow>).eq("id", id).select("*").maybeSingle();
